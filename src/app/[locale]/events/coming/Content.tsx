...
export default function Content() {
	...
	const handleSearchSubmit = () => {
		...
		router.push(`coming?page=1&search=${searchInput}`);
	};
	...
	useEffect(() => {
		if ((page && isNaN(Number(page))) || !page) router.push("coming?page=1" + (search ? `&search=${search}` : ""));

		setEvents(getNextEvents(10, Number(page) || 1, search || "", locale));
		setLoading(false);
  ...
  return (
		<RoundContainer className={styles.container}>
				<div className={styles.title}>
					...
					<p className={styles.subtitle}>{t("NextEvents.Description")}</p>
				</div>

				<div className={styles.horizontalBar}></div>

				<div className={styles.informations}>
					<SearchBar searchButton={true} placeholder={t("Placeholder")} onValidate={handleSearchSubmit} onChange={handleSearch} searchInput={searchInput} />
					...
					<div className={styles.noEvents}>
						{!loading ? (
							(search && search === "") || !search ? (
								<>
									<p>{t("NextEvents.Nothing1")}</p>
									<p>{t("NextEvents.Nothing2")}</p>
								</>
							) : (
								<>
									<p>{t("NotFound1")}</p>
				...
				<ReactPaginate
					...
					onPageChange={(e) => {
						router.push(`coming?page=${e.selected + 1}` + (search ? `&search=${search}` : ""));
					}}
				/>
